package players.assign;

import core.GameState;
//import players.heuristics.AdvancedHeuristic;
//import players.heuristics.CustomHeuristicNew;
...
public class SingleTreeNode
{
    ...
    private double win_sqaure;
    private double meansqaue;

    SingleTreeNode(MCTSParamsNew p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNode(MCTSParamsNew p, SingleTreeNode parent, int childIdx, Random rnd, int num_actions,
    ...
    void setRootGameState(GameState gs)
    {
        ...
        if (params.heuristic_method == params.CUSTOM_HEURISTIC)
            this.rootStateHeuristic = new CustomHeuristicNew(gs);
        ...
        else if (params.heuristic_method == params.winstate) // New method: combined heuristics
            this.rootStateHeuristic = new WinScoreHeuristic();
        else if (params.heuristic_method == params.playercount) // New method: combined heuristics
    ...
    private SingleTreeNode treePolicy(GameState state) {
        ...
        while (!state.isTerminal() && cur.m_depth < params.rollout_depth)
        {
            if (cur.notFullyExpanded()) {
              ...
              cur = cur.uct(state);         //progressive bias
              // cur=cur.rave(state, 10);  //rave
               //cur=cur.uctTUNED(state);        // ucb tuned with bandit algorithm
            }
        }
    ...
    private SingleTreeNode uctTUNED(GameState state) {
        SingleTreeNode selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNode child : this.children)
        {
            ...
            double variance = Math.abs((1 /this.nVisits) * (this.win_sqaure) - this.meansqaue);
            double  valueodS = variance + Math.sqrt((2 * Math.log(this.nVisits + 1))/(child.nVisits + params.epsilon));

            double uctValue = childValue +
                    params.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + params.epsilon)* Math.min(0.25, valueodS)) ;
    ...
    private SingleTreeNode rave(GameState state, double valueb) {
        ...
        for (SingleTreeNode child : this.children) {
           ...
           childValue = Utils.normalise(childValue, bounds[0], bounds[1]);
            ...
            double amafscores = child.totValue / (child.nVisits + params.epsilon);



            double alpha = Math.max(0, (valueb - child.nVisits) / valueb);

            double raveValue = (1 - alpha) * childValue + (alpha * amafscores);// + (rootStateHeuristic.evaluateState(state)/(1 + child.nVisits + params.epsilon));
    ...
    private double rollOut(GameState state)
    {
        ...
        return rootStateHeuristic.evaluateState(state);// * Math.pow(0.1,thisDepth); //reward decay
    }
    ...
    private void backUp(SingleTreeNode node, double result)
    {
        ...
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
            n.win_sqaure += Math.pow(n.totValue, 2);
            n.meansqaue += Math.pow(n.totValue/n.nVisits, 2);
